...
import { MessageCircle, X, Send, Loader2, Bot, CheckCircle, Phone, MessageSquare } from 'lucide-react';

const G = '#1b4332';
const PRAJAKTA_PHONE = '7012536289';
...
function parseTime(text: string): { start: string; end: string } {
  ...
  if (!period && h < 6) h += 12; // assume pm for ambiguous times like "7" → 7pm
...
function extractStudentName(text: string): string {
  // "with Anjali" / "for Anjali Sharma" / "student Anjali"
  const patterns = [
...
function isClassIntent(text: string): boolean {
  ...
  return (
    ...
    (lower.includes('class') || lower.includes('session') || lower.includes('personal') || lower.includes('group') || lower.includes('batch'))
  );
}

// ── Notification trigger ──────────────────────────────────────
async function triggerNotification(data: {
  teacherName: string; date: string; time: string;
  type: string; studentName?: string; className?: string;
}) {
  const res = await fetch('/api/notify-teacher', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      phone: PRAJAKTA_PHONE,
...
// ── Types ────────────────────────────────────────────────────
interface Msg { role: 'user' | 'yogi'; text: string; ts: Date; extra?: React.ReactNode; }
...
export function StaffYogi() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([{
    role: 'yogi',
    text: `Namaste Priyanka! 🙏 I'm Yogi, your staff assistant.\n\nI can help you:\n• **Assign a class** for Prajakta (personal or group)\n• **Send notifications** to Prajakta via call, SMS & WhatsApp\n• **Check today's schedule**\n\nTry saying:\n_"Assign personal session for Prajakta tomorrow at 6am with Anjali"_\n_"Schedule group class Monday 8am"_`,
    ts: new Date(),
  }]);
  ...
  const addMsg = (role: 'user' | 'yogi', text: string, extra?: React.ReactNode) => {
    setMsgs(m => [...m, { role, text, ts: new Date(), extra }]);
  };
  ...
  const handleSend = async () => {
    ...
    if (pendingClass && (lower.includes('yes') || lower.includes('confirm') || lower.includes('ok') || lower.includes('assign') || lower.includes('send'))) {
      const cls = pendingClass;
      setPendingClass(null);

      // Create class in store
      ...
      if (prajakta) {
        ...
        addMsg('yogi', `✅ Class assigned! Now notifying Prajakta...\n\n📞 Calling +91 ${PRAJAKTA_PHONE}\n📱 Sending SMS\n💬 Sending WhatsApp`);

        // Trigger notification
        try {
          const result = await triggerNotification({
            teacherName: prajakta.name,
            date: new Date(cls.date + 'T12:00:00').toLocaleDateString('en-IN', { weekday:'long', day:'numeric', month:'long' }),
            time: `${cls.time_start} – ${cls.time_end}`,
            type: cls.type,
            studentName: cls.student_name,
            className: cls.batch_name,
          });
          setNotifResult(result);

          const callOk = result?.results?.call?.status === 'sent';
          const smsOk  = result?.results?.sms?.status === 'sent';
          const waOk   = result?.results?.whatsapp?.status === 'sent';

          addMsg('yogi',
            `Notifications sent! Here's the status:\n\n` +
            `📞 Call: ${callOk ? '✅ Placed' : '⚠️ Failed (Twilio not configured?)'}\n` +
            `📱 SMS: ${smsOk ? '✅ Sent' : '⚠️ Failed'}\n` +
            `💬 WhatsApp: ${waOk ? '✅ Sent' : '⚠️ Failed'}\n\n` +
            ((!callOk && !smsOk && !waOk) ? `_Note: Add Twilio credentials in Vercel dashboard to enable real notifications._` : `_Prajakta will receive reminders until she marks the class on the portal._`)
          );
        } catch (e) {
          addMsg('yogi', `Class assigned in portal ✅\n\n⚠️ Could not reach notification service. Please ensure Twilio is configured in Vercel environment variables.`);
        }
      }
      setLoading(false);
      return;
    ...
    // Cancel pending
    if (pendingClass && (lower.includes('no') || lower.includes('cancel') || lower.includes('wrong'))) {
      setPendingClass(null);
      addMsg('yogi', `No problem! Let me know what you'd like to assign instead.`);
      setLoading(false);
      return;
    }
    ...
    if (isClassIntent(text)) {
      ...
      const typeLabel = parsed.type === 'personal' ? '👤 Personal Session' : '👥 Group Class';
      const detailLabel = parsed.type === 'personal'
        ? (parsed.student_name ? `Student: ${parsed.student_name}` : 'Student: (not specified)')
        : `Class: ${parsed.batch_name}`;
      ...
      addMsg('yogi',
        ...
        `${parsed.type === 'personal' ? '👤' : '📚'} ${detailLabel}\n\n` +
        `After assigning, I'll:\n📞 Call Prajakta (+91 ${PRAJAKTA_PHONE})\n📱 Send SMS\n💬 Send WhatsApp\n\n` +
        `**Shall I proceed?** _(yes / no)_`
      );
      setLoading(false);
      return;
    }
    ...
    // Schedule query
    if (lower.includes('schedule') || lower.includes('today') || lower.includes("prajakta's class") || lower.includes('assigned')) {
      const teachers = portalDB.getUsers().filter(u => u.role === 'teacher');
      ...
      if (prajakta) {
        ...
        if (todayClasses.length === 0 && upcoming.length === 0) {
          ...
          addMsg('yogi', todayText + upcomingText);
        }
      }
      setLoading(false);
      return;
    ...
    addMsg('yogi',
      ...
      `**📋 Check schedule:**\n_"What classes does Prajakta have today?"_\n_"Show Prajakta's schedule this week"_\n\n` +
      `After assigning, I automatically call, SMS & WhatsApp Prajakta at +91 ${PRAJAKTA_PHONE}. 🙏`
    );
      ...
      {open && (
        <div style={{
          ...
          <div style={{ padding: '14px 16px', background: `linear-gradient(135deg, ${G}, #2d6a4f)`, display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
            ...
            <div>
              <p style={{ fontSize:14, fontWeight:700, color:'#fff' }}>Yogi — Staff Assistant</p>
              <p style={{ fontSize:11, color:'rgba(255,255,255,0.7)' }}>Assign classes · Notify Prajakta</p>
            </div>
          </div>
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'16px 14px', display:'flex', flexDirection:'column', gap:10 }}>
            {msgs.map((msg, i) => (
              <div key={i} style={{ display:'flex', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row', gap:8, alignItems:'flex-end' }}>
                ...
                <div style={{
                  maxWidth: '80%', padding: '10px 12px', borderRadius: msg.role === 'user' ? '12px 12px 2px 12px' : '12px 12px 12px 2px',
          ...
          <div style={{ padding:'10px 12px', borderTop:'1px solid #f3f4f6', display:'flex', gap:8, flexShrink:0 }}>
            ...
            <button onClick={handleSend} disabled={!input.trim() || loading}
              style={{ width:40, height:40, borderRadius:10, border:'none', cursor: input.trim() ? 'pointer' : 'default', background: input.trim() ? G : '#e8e2da', display:'flex', alignItems:'center', justifyContent:'center', transition:'background 0.2s', flexShrink:0 }}>
